...
const PortfolioCard = ({ content }) => {
  return (
    <div
      className={`${classes.card} rounded-tr rounded-tl flex flex-col md:mb-0 mb-5 md:w-full w-3/4 mx-auto`}
    >
      ...
      <div
        className={`${classes["card-bg"]} rounded-br rounded-bl w-full text-center py-3`}
      >
        <h3>{content.name}</h3>
      </div>
    </div>
